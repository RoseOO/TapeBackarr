...
func TestAdminOnlyMiddlewareNoClaims(t *testing.T) {
	...
	handler := s.adminOnlyMiddleware(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))

	// Request with no claims in context — should return 401, not panic
	req := httptest.NewRequest("GET", "/", nil)
	rr := httptest.NewRecorder()
...
func TestAdminOnlyMiddlewareValidClaims(t *testing.T) {
	...
	tests := []struct {
		...
		{"operator denied", models.RoleOperator, http.StatusForbidden},
		{"readonly denied", models.RoleReadOnly, http.StatusForbidden},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			claims := &auth.Claims{UserID: 1, Username: "test", Role: tt.role}
			req := httptest.NewRequest("GET", "/", nil)
			...
			if rr.Code != tt.wantStatus {
				t.Errorf("expected status %d for role %s, got %d", tt.wantStatus, tt.role, rr.Code)
			}
		})
	}
}
